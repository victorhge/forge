...
`posted-ids` is a `let`-bound variable captured by the macro. Because `push` in
Elisp does not mutate a callee's binding, tracking is done inline via a
`forge-itest--record` macro (not a function) that expands to a `push` into the
macro-captured variable:
...
- GitLab: `DELETE /projects/:id/merge_requests/:iid/notes/:note_id`
  (GitLab inline comments are posted as discussion notes; the returned `id` from
  `forge-itest--gl-add-review-comment` is the note ID of the first note in the
  discussion. Reply notes returned by `forge-itest--gl-reply-to-discussion` also
  carry a top-level `id` that is the note ID directly.)
