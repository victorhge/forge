...
- **`forge-{github,gitlab,forgejo,gitea,gogs,bitbucket}.el`** — Per-forge repository subclasses with URL format slots, and forge-specific `forge--pull` / `forge--update-*` method implementations. `forge-github.el` and `forge-gitlab.el` also implement the review-comment generics declared in `forge-review.el` (`forge--update-pullreq-review-comments`, `forge--update-pullreq-draft-notes`, `forge--review-create-draft`, `forge--review-edit-draft`, `forge--review-publish-pending`, `forge--review-post-reply`, `forge--review-set-thread-resolved`, `forge--review-delete-comment`, `forge--review-post-comment`, `forge--submit-add-review-reply`, `forge--submit-add-single-review-comment`).
...
- **API calls in write methods**: backend `cl-defmethod` implementations call `forge--query` (GitHub GraphQL) or `forge--rest` (GitLab) directly. Tests use fake subclasses (`forge-test-github-repository`, `forge-test-gitlab-repository`) that stub leaf primitives (`forge--review-post-reply`, `forge--review-post-comment`, `forge--review-create-draft`, `forge--review-edit-draft`, `forge--review-publish-pending`, etc.) at the CLOS dispatch layer, or stub `forge--query`/`forge--rest` via `cl-letf` for payload verification.
- **Pending comments**: `pending-p t` means the comment exists on the server as a draft (author-only visible) and `their-id` is always non-nil. Every comment row has a real server-assigned `their-id` — there are no longer any "locally staged only" rows. `forge-submit-pending-review` publishes the batch via `forge--review-publish-pending`. `forge-add-single-review-comment` posts immediately (bypassing the draft flow) with `pending-p nil`.
- **Post-submit async**: All review write methods use `:callback`/`:errorback`, matching the async pattern used throughout the codebase. All destructive work (`closql-delete`, `forge--pull-topic`, buffer refresh, `oset` local state) runs inside `:callback` only. `:errorback` is always `(forge--post-submit-errorback)` — signals an error and leaves all state intact so the user can retry.
- **GitHub review GraphQL mutations**: `forge--review-create-draft` uses `addPullRequestReviewThread`; `forge--review-edit-draft` uses `updatePullRequestReviewComment` (`rc.their-id` = comment node ID); `forge--review-publish-pending` uses a two-step flow: query `reviews(last:1 states:[PENDING])` to get the review node ID, then `submitPullRequestReview`; `forge--review-post-comment` uses `addPullRequestReviewThread`; `forge--review-post-reply` uses `addPullRequestReviewThreadReply` (`rc.discussion-id` = thread node ID); `forge--review-delete-comment` uses `deletePullRequestReviewComment` (`rc.their-id` = comment node ID). All IDs are stored in the DB from the pull mapping.
- **GitLab review methods**: use `forge--rest` (POST/PUT/DELETE), matching all other GitLab write methods. `forge--review-create-draft` POSTs to `/draft_notes`; `forge--review-edit-draft` PUTs to `/draft_notes/:number`; `forge--review-delete-comment` dispatches on `pending-p` (draft → `/draft_notes/:number`, submitted → `/notes/:number`); `forge--review-publish-pending` POSTs to `/draft_notes/bulk_publish`. `forge--update-pullreq-draft-notes` is called from the pull flow after `forge--update-pullreq-review-comments` to fetch and store server-side drafts.
...
- **Submit callback protocol**: all functions assigned to `forge--submit-post-function` must accept exactly two arguments `(repo post)`. Post-buffer staging callbacks (`forge-review--stage-comment`, `forge-review--save-comment-edit`) call the forge API immediately; they capture buffer-local vars (`forge--pre-post-buffer`, `current-buffer`) before the async call and do all DB/buffer work inside `:callback`. API-submitting callbacks are `cl-defmethod` generics specialised on the repo class.
- **Post buffer actions for `new-review-comment`**: `C-s` → `forge-post-stage` (calls `forge--review-create-draft`; stores with `pending-p t`); `C-c C-c` → `forge-post-submit` (posts immediately as single comment with `pending-p nil`); `C-c C-p` → `forge-post-stage-and-publish` (creates draft then immediately publishes the entire pending batch; only shown when the PR already has pending comments).
...
| `forge-review--FOO` | Internal to `forge-review.el`; staging helpers (`forge-review--stage-comment`, `forge-review--save-comment-edit`, `forge-review--stage-and-publish`) conform to the `(repo post)` submit protocol and call the forge API immediately |
