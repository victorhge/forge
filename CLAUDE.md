...
- **Post buffer actions for `new-review-comment`**: `C-s` → `forge-post-stage` (calls `forge--review-create-draft`; stores with `pending-p t`); `C-c C-c` → `forge-post-submit` (posts immediately as single comment with `pending-p nil`). To publish all pending drafts, return to the topic buffer and use `/v` (`forge-submit-pending-review`).
...
| `forge-review--FOO` | Internal to `forge-review.el`; staging helpers (`forge-review--stage-comment`, `forge-review--save-comment-edit`) conform to the `(repo post)` submit protocol and call the forge API immediately |
